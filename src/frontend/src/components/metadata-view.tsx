      ...
      explanation: (
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-base text-muted-foreground">
            <p><span className="text-blue-500 dark:text-blue-400 font-medium">Input tokens:</span> {inputTokens.toLocaleString()} tokens the model actually compute (after cache deduction)</p>
            <p><span className="text-amber-500 dark:text-amber-400 font-medium">Cache hits:</span> {cacheHits.toLocaleString()} tokens served from cache without recomputing</p>
            {cacheHits > 0 && <p><span className="text-pink-500 dark:text-pink-400 font-medium">Total prompt:</span> {totalInputTokens.toLocaleString()} total tokens in the prompt</p>}
            ...
            <p><span className="text-cyan-500 dark:text-cyan-400 font-medium">Computed total:</span> {computedTokens.toLocaleString()} tokens actually processed ({cacheHits > 0 ? `${cacheHits.toLocaleString()} saved by cache` : "no cache savings"})</p>
          </div>
        </div>
      ),
    },
    {
      ...
      explanation: (
        <div className="space-y-2">
          ...
          <div className="space-y-1 text-base text-muted-foreground">
            <p><span className="text-blue-500 dark:text-blue-400 font-medium">Prompt processing:</span> {timings?.prompt_per_second ? `${timings.prompt_per_second.toFixed(2)} tok/s` : "N/A"} — how fast input tokens are processed</p>
            <p><span className="text-green-500 dark:text-green-400 font-medium">Token generation:</span> {timings?.predicted_per_second ? `${timings.predicted_per_second.toFixed(2)} tok/s` : "N/A"} — how fast output tokens are generated</p>
          </div>
        </div>
      ),
    },
